...
def fault_CMT(lat,lon,depth,M,strikeF,dipF,nstr,ndip):
	...
	for j in range (0, ndip):
		for i in range (0, nstr):
			...
			(fault_lon[k], fault_lat[k]) = coord_tools.utm2ll(fault_X*1000,fault_Y*1000,-71)

			(fault_lon1[k], fault_lat1[k]) = coord_tools.utm2ll(fault_X1*1000,fault_Y1*1000,-71)
			(fault_lon2[k], fault_lat2[k]) = coord_tools.utm2ll(fault_X2*1000,fault_Y2*1000,-71)
			(fault_lon3[k], fault_lat3[k]) = coord_tools.utm2ll(fault_X3*1000,fault_Y3*1000,-71)
			(fault_lon4[k], fault_lat4[k]) = coord_tools.utm2ll(fault_X4*1000,fault_Y4*1000,-71)
			k = k+1

	return (fault_lon, fault_lat, fault_alt, strike, dip, DLEN*numpy.ones([nstr*ndip,1]), DWID*numpy.ones([nstr*ndip,1]), fault_lon1, fault_lat1,fault_lon2, fault_lat2, fault_lon3, fault_lat3, fault_lon4, fault_lat4)
