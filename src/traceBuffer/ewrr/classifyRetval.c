...
int traceBuffer_ewrr_classifyGetRetval(const int retval)
{
    ...
    if (retval == GET_MISS)
    {   
        sprintf(msg, "%s: Some messages were missed", __func__);
        // LOG_MSG("%s", msg);
        LOG_WARNMSG("%s", msg);
        return -1; 
    }
    ...
    {   
        sprintf(msg, "%s: Message exceeded NTRACK_GET", __func__);
        // LOG_MSG("%s", msg);
        LOG_WARNMSG("%s", msg);
        return -1; 
    }   
    ...
    {
        sprintf(msg,
               "%s: Next message of requested logo(s) is too big", __func__);
        // LOG_MSG("%s", msg);
        LOG_WARNMSG("%s", msg);
        return -2;
    }
    ...
    {
        sprintf(msg, "%s: Some messages were overwritten", __func__);
        // LOG_MSG("%s", msg);
        LOG_WARNMSG("%s", msg);
        return -1;
    }
    ...
    {
        sprintf(msg, "%s: A gap in messages was detected", __func__);
        // LOG_MSG("%s", msg);
        LOG_WARNMSG("%s", msg);
        return -1;
    }
    // I don't know
    sprintf(msg, "%s: Could not classify return value", __func__);
    LOG_MSG("%s", msg);
    LOG_WARNMSG("%s", msg);
    return -2;
}
