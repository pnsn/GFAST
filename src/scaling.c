                      ...
                      double *M, double *VR)
{
    ...
    *M = 0.0;
    ...
    ierr = numpy_lstsq(LAPACK_ROW_MAJOR, 
                       n, 1, 1, WG, Wb,
                       NULL, M1, NULL, NULL); 
    if (ierr != 0){
        if (verbose > 0){
    ...
    res = 0.0;
    for (i=0; i<n; i++){
        res = res + (pow(10.0, b[i]) - pow(10.0, UP[i]));
        b10 = b10 + pow(10.0, b[i]);
    }
    // Copy results
    *M = M1[0];
    *VR = ((1.0 - sqrt(res))/sqrt(b10))*100.0;
    // Free space
ERROR:;
    if (b  != NULL){free(b);}
    if (G  != NULL){free(G);}
...
//============================================================================//
/*!
 * @brief Sets the forward modeling matrix G 
 *
 ...
 *
 * @param[out] G       forward modeling matrix for PGD [n x 1]
 *
 * @result 0 indicates success
 *
 */
