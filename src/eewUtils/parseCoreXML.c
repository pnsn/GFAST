                          ...
                          struct GFAST_shakeAlert_struct *SA)
{
  ...
  while (event_xml != NULL)
    {
      ...
      //retrieve source and version
      orig_sys=xmlGetProp(event_xml,(xmlChar*)"orig_sys");
      msg_version=xmlGetProp(event_xml,(xmlChar*)"version");
  ...
  if (ierr != 0)
  ...
  else
    {
      ...
      if (orig_sys!=NULL)  strncpy(SA->orig_sys, (char *)orig_sys, 10);
      //message version
      if (msg_version!=NULL)  SA->version = atoi((char *)msg_version);
      // origin time
